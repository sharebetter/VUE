...
export default {
  ...
  methods: {
    ...
    getPrice () {
     ...
     this.price = parseFloat((this.buyType.value+1) * (this.period.value+1))*250;
     let arr = reqParams.version.split(',')
     arr.forEach(element => {
          this.price=(Number(element)+1)*100 + this.price
     });
     this.price=(this.buyNum)*this.price
    },
    showPayDialog () {
    ...
    confirmBuy () {
      ...
      axios.get('/static/data/db.json', reqParams)
      .then((res) => {
