<template>
  <div>
    <vheader></vheader>
    <vbrander><span>Cart</span></vbrander>
              ...
              <li>
                <div class="cart-tab-1">
                  <div class="cart-item-check">
                    ...
                    <a href="javascipt:;" class="checkbox-btn item-check-btn" v-bind:class="{'check':item.checked=='1'}" @click="goodsEdit('checked',item)">
                  ...
                  <div class="cart-item-title">
                    <div class="item-name">{{item.productImage}}</div>
                  </div>
                </div>
                  ...
                  <div class="item-quantity">
                    <div class="select-self select-self-open">
                      <div class="select-self-area">
                        <a class="input-sub" @click="goodsEdit('des',item)">-</a>
                        <span class="select-ipt">{{item.productNum}}</span>
                        <a class="input-add" @click="goodsEdit('add',item)">+</a>
                      </div>
                    </div>
                  </div>
                </div>
                <div class="cart-tab-4">
                  <div class="item-price-total">{{parseFloat(item.salePrice*item.productNum)|currency('￥')}}</div>
                </div>
    ...
    <modal :mdShow="modalConfirm" @close="closeModal">
      ...
      <div slot="btnGroup">
        <a class="btn btn--m" href="javascript:;" @click="cartDelt">确认</a>
    ...
    export default{
        ...
        computed: {
          ...
          totalPrice () {
          	...
          	this.cartList.forEach((item)=>{
              if(item.checked=='1'){
                allPrice += parseFloat(item.salePrice * item.productNum)              	
              }
            })
          	return allPrice;
          }         
        ...
        methods: {
        	getCartInfo () {
        		axios.get('/users/cart').then((result)=> {
        			let res = result.data
        			if(res.status == 0){
        	...
        	goodsEdit (edit,item) {
        		...
        		axios.post('/users/cartEdit',{
        			...
        			let res = result.data
        			if(res.status == 0){ 
        				this.$store.commit("updateCartCount",edit=="add" ? 1:-1)
        				this.getCartInfo ()        				
        			}else{
        				alert('失败')
        			}
        	...
        	delConfirm (item) {
        	  ...
        	  this.delNum =  item.productNum
        	},
        	closeModal () {
        	  this.modalConfirm = false;
        	},
        	cartDelt () {
        		axios.post('/users/cartDelt',{
        	...
        	selectAll () {
        		...
        		axios.post('/users/checkAll',{
        			...
        			if(res.status==0){
        				this.cartList.forEach(item => {
        			     item.checked=this.checkAll==1?1:0
        		    })
        			}else{
        				alert('checkAll失败')
        			}
