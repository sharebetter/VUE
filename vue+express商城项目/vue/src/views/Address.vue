<template>
    <div>
      <vheader></vheader>
      <vbrander><span>Address</span></vbrander>
        ...
        <div class="container">
          <div class="checkout-addr">
              ...
              <div class="addr-list">
                <ul>
                  <li v-for="(item,index) in addressListFilter" :class="{'check': checkIndex==index}" @click="checkIndex=index;">
                    ...
                    <div class="addr-opration addr-default" v-if="!item.isDefault" @click="addressDefault(item.addressId)">
            ...
            <div class="next-btn-wrap">
              <router-link class="btn btn--m btn--red" v-bind:to="{path:'orderConfirm',query:{'addressId': defaultAddress}}">Next</router-link>
            </div>
          </div>
        </div>
      </div>
      <modal v-bind:mdShow="mdAddressConfirm" v-on:close="closeModal">
        <p slot="message">
          <svg class="icon-status-ok">
            <use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#icon-status-ok"></use>
          </svg>
          <span>您真的要删除该地址吗？</span>
        </p>
        <div slot="btnGroup">
          <a class="btn btn--m" href="javascript:;" @click="delAddress">确认</a>
          <a class="btn btn--m" href="javascript:;" @click="mdAddressConfirm=false">取消</a>
        </div>
      </modal>
  ...
  export default{
      ...
      methods: {
      	...
      	addressDefault (addressId) {
           axios.post('/users/setAddressDefault',{
           	  addressId: addressId
           }).then( result => {
           	 let res = result.data
           	 if(res.status==0){
